...
class ECUCAnalyzer(BaseAnalyzer):
    ...
    def _parse_module(self, element: etree.Element, namespaces: Dict[str, str]) -> Optional[ECUCModule]:
        ...
        try:
            ns = '{http://www.autosar.org/schema/r4.0}'
            # Extract module name - look for direct child first, then descendant
            short_name = element.find(f"{ns}SHORT-NAME")
            ...
            # Extract containers - look in CONTAINERS element
            containers_elem = element.find(f"{ns}CONTAINERS")
            if containers_elem is not None:
                containers = containers_elem.findall(f"{ns}ECUC-CONTAINER-VALUE")
                for container in containers:
                    container_data = self._parse_container(container, namespaces)
                    if container_data:
                        module.containers.append(container_data)
            
            # Extract module-level parameters (if any)
            param_values_elem = element.find(f"{ns}PARAMETER-VALUES")
            if param_values_elem is not None:
                params = param_values_elem.findall(f"{ns}ECUC-NUMERICAL-PARAM-VALUE") + \
                         param_values_elem.findall(f"{ns}ECUC-TEXTUAL-PARAM-VALUE")
                for param in params:
                    param_data = self._parse_parameter(param, namespaces)
                    if param_data:
                        module.parameters.append(param_data)
            
            # Extract module-level references (if any)
            ref_values_elem = element.find(f"{ns}REFERENCE-VALUES")
            if ref_values_elem is not None:
                refs = ref_values_elem.findall(f"{ns}ECUC-REFERENCE-VALUE")
                for ref in refs:
    ...
    def _parse_container(self, element: etree.Element, namespaces: Dict[str, str]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Extract nested parameters from PARAMETER-VALUES
            param_values = element.find(f"{ns}PARAMETER-VALUES")
            if param_values is not None:
                params = param_values.findall(f"{ns}ECUC-NUMERICAL-PARAM-VALUE") + \
                         param_values.findall(f"{ns}ECUC-TEXTUAL-PARAM-VALUE")
                for param in params:
                    param_data = self._parse_parameter(param, namespaces)
                    if param_data:
                        container['parameters'].append(param_data)
            
            # Extract nested references from REFERENCE-VALUES
            ref_values = element.find(f"{ns}REFERENCE-VALUES")
            if ref_values is not None:
                refs = ref_values.findall(f"{ns}ECUC-REFERENCE-VALUE")
                for ref in refs:
                    ref_data = self._parse_reference(ref, namespaces)
                    if ref_data:
                        container['references'].append(ref_data)
            
            # Extract nested containers from SUB-CONTAINERS
            sub_containers_elem = element.find(f"{ns}SUB-CONTAINERS")
            if sub_containers_elem is not None:
                sub_containers = sub_containers_elem.findall(f"{ns}ECUC-CONTAINER-VALUE")
    ...
    def _parse_parameter(self, element: etree.Element, namespaces: Dict[str, str]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ns = '{http://www.autosar.org/schema/r4.0}'
            definition_ref = element.find(f"{ns}DEFINITION-REF")
            ...
            # Check if symbolic
            param['is_symbolic'] = element.find(f"{ns}IS-SYMBOLIC") is not None or \
                                  element.find(f".//{ns}IS-SYMBOLIC") is not None
    ...
    def _parse_reference(self, element: etree.Element, namespaces: Dict[str, str]) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            value_ref = element.find(f"{ns}VALUE-REF")
    ...
    def _check_references(self, result: AnalysisResult) -> None:
        ...
        for module in self._modules.values():
            ...
            # Container-level references
            for container in module.containers:
                self._check_container_references(container, module.name, ref_stats, all_refs)
    ...
    def _check_container_references(self, container: Dict[str, Any], module_name: str, 
                                   ref_stats: Dict[str, Any], all_refs: Dict[str, List[str]]) -> None:
        """Recursively check container references.
        
        Args:
            container: Container data
            module_name: Parent module name
            ref_stats: Reference statistics to update
            all_refs: All references dictionary to update
        """
        # Check container's references
        for ref in container.get('references', []):
            ...
            if ref_path:
                # Store reference for circular dependency check
                if module_name not in all_refs:
                    all_refs[module_name] = []
                all_refs[module_name].append(ref_path)
        ...
        # Recurse into sub-containers
        for sub_container in container.get('sub_containers', []):
            self._check_container_references(sub_container, module_name, ref_stats, all_refs)
    ...
    def _find_patterns(self, document: ARXMLDocument, result: AnalysisResult) -> None:
        ...
        ns = '{http://www.autosar.org/schema/r4.0}'
        module_elems = document.root.findall(f".//{ns}ECUC-MODULE-CONFIGURATION-VALUES")
        container_elems = document.root.findall(f".//{ns}ECUC-CONTAINER-VALUE")
        
        module_names = []
        for elem in module_elems:
            name_elem = elem.find(f"{ns}SHORT-NAME")
            if name_elem is not None:
                module_names.append(name_elem)
        
        container_names = []
        for elem in container_elems:
            name_elem = elem.find(f"{ns}SHORT-NAME")
            if name_elem is not None:
                container_names.append(name_elem)
